...
uint8_t GetBatteryLevel(void)
{
  ...
  return batteryLevel;  /* 30 = 3 Volts */
}
...
void SetDevAddr(uint32_t daddr){
	devaddr = daddr;
}

uint32_t GetDevAddr(void){
	/*
	uint32_t ID_2_val = HAL_GetUIDw2();
  uint32_t ID_0_val = HAL_GetUIDw0();
  uint32_t tmp;
	union{
		uint32_t addr;
		uint8_t	byte[4];
  ...
  data[0].byte[0] = (ID_0_val);
  data[0].byte[1] = (ID_0_val) >> 8;
  data[0].byte[2] = 0;
  data[0].byte[3] = 0;

  data[1].byte[0] = (ID_2_val);
  data[1].byte[1] = (ID_2_val) >> 16;
  data[1].byte[2] = 0;
  data[1].byte[3] = 0;

  tmp = data[0].addr * data[1].addr;
